...
# K230 -> ESP32-P4 one-way UART result output.
# Wire K230 IO32(TX) to ESP32-P4 UART RX, and connect GND together.
K230_UART_BAUD = 115200
K230_UART_TX_IO = 32
K230_UART_RX_IO = 33
K230_REPORT_INTERVAL_MS = 2000
K230_SENSOR_ROTATE_180 = True
# Keep AI input unchanged. The sensor stream is already rotated by hmirror+vflip.
# Per-frame image copy/flip is unstable on CanMV K230 and can raise IDE interrupt.
K230_AI_INPUT_ROTATE_180 = False
...
def init_fall_uart():
    try:
        ...
        fpioa = FPIOA()
        fpioa.set_function(K230_UART_TX_IO, FPIOA.UART3_TXD)
        fpioa.set_function(K230_UART_RX_IO, FPIOA.UART3_RXD)
        uart_dev = UART(UART.UART3, K230_UART_BAUD)
        print("Fall UART ready: IO%d=TX IO%d=RX UART3 @ %d" %
              (K230_UART_TX_IO, K230_UART_RX_IO, K230_UART_BAUD))
        return uart_dev
    except Exception as e:
        print("machine UART init failed:", e)
        return None
...
def send_fall_state(state, force=False):
    global last_report_state, last_report_ms

    now = now_ms()
    if (not force and state == last_report_state and
            ms_diff(now, last_report_ms) < K230_REPORT_INTERVAL_MS):
        return
    ...
    if fall_uart is None:
        print("UART not ready:", line.strip())
        return

    try:
        if hasattr(fall_uart, "write"):
            try:
                fall_uart.write(line)
            except TypeError:
                fall_uart.write(line.encode())
        else:
            fall_uart.send(line)
    except Exception as e:
        print("UART send failed:", e)

def create_camera_sensor(width, height):
    sensor = Sensor(width=width, height=height)
    sensor.reset()

    if K230_SENSOR_ROTATE_180:
        # Rotate the sensor/display stream before PipeLine starts.
        sensor.set_hmirror(True)
        sensor.set_vflip(True)
        print("Sensor/display rotate 180 enabled before sensor run")

    return sensor

def rotate_frame_180_for_ai(img):
    if not K230_AI_INPUT_ROTATE_180:
        return img

    try:
        out = img.copy() if hasattr(img, "copy") else img
        rotated = False
        if hasattr(out, "hmirror"):
            ret = out.hmirror()
            if ret is not None:
                out = ret
            rotated = True
        if hasattr(out, "vflip"):
            ret = out.vflip()
            if ret is not None:
                out = ret
            rotated = True
        if rotated:
            return out
        if hasattr(out, "rotation_corr"):
            ret = out.rotation_corr(z_rotation=180)
            if ret is not None:
                out = ret
        return out
    except Exception as e:
        print("AI input rotate 180 failed:", e)
        return img

fall_uart = init_fall_uart()
send_fall_state("NOFALL", force=True)
        ...
        with ScopedTiming("display_draw", self.debug_mode > 0):
            if dets:
                ...
                for det_box in dets:
                    ...
                    pl.osd_img.draw_rectangle(x1, y1, int(w), int(h), color=self.color[cls_id], thickness=2)
                    pl.osd_img.draw_string_advanced(
                        x1, y1-50, 32,
                        " " + label + " " + str(score),  # 类别名称和置信度 / Category name and confidence
                        color=self.color[cls_id]
                    )
                    if label == "Fall":
                        fall_state = "FALL"

                send_fall_state(fall_state)
            else:
                send_fall_state("NOFALL")
                # 如果没有检测结果，清除OSD图像
...
if __name__ == "__main__":
    ...
    sensor = create_camera_sensor(rgb888p_size[0], rgb888p_size[1])
    ...
    while True:
        with ScopedTiming("total",0):
            ...
            img = pl.get_frame()
            ai_img = rotate_frame_180_for_ai(img)

            # 推理当前帧
            # Infer current frame
            res = fall_det.run(ai_img)
